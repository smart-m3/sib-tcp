...
#define BACKLOG 1000000     // how many pending connections queue will hold
...
static int listener_handle_subscribe(Listener *listener, Client *client, NodeMsgContent_t *msg)
{
    ...
    if(bd && waiter && waiter->rsp && (parseSSAPmsg_get_msg_status(waiter->rsp)==2))
    {
    ...
    else  if(bd && waiter && waiter->rsp && (parseSSAPmsg_get_msg_status(waiter->rsp)!=2))
    {
...
gint client_send_message( int fd, guchar *buf, gint len)
{
    ...
    while(total < len)
    {
        ...
        ///
        #ifdef HIPDEX
        n = send(fd, cipher+total, bytesleft, SEND_FLAGS);
        #else
